...
{
    public class FunctionIntWithError : IFunction<int>
    {
        public IFunctionResult<int> Run()
        {
            var result = this.MakeResult();

            result.Messages.Add(new Message { Code = 100, Description = "Error -> FunctionIntOnError", Severity = MessageType.Error });           
